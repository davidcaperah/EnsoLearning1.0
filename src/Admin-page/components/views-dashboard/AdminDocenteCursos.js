...
const AdminDocenteCursos = () => {
    ...
    useEffect(()=>{
        const sendData = async ()=>{
            ...
            const response = await api.post('/api-php-react/Cargar_cursos.php', DatosJson);
            setDatosRecibidos(
                ...DatosRecibidos,
                response.data
            ) 
    ...
    async function NoRecargar (e){
        e.preventDefault()
        const result = DatosRecibidos.filter(data => data.id === parseInt(Campos.curso));
    ...
    return(
        <div className="p-4" >
            <h5> Aquí podrás selecionar la materia y el curso que este docente dictará en tu colegio, podrás agregar más de un curso y más de una materia a cada docente. </h5>
            <form className="mt-2" onSubmit={NoRecargar}  >
                <p className="m-2" > Seleciona el curso que le quieres asignar a este docente. </p>
                <select className="form-control m-2" onChange={onChange} name="curso" >
                    <option> Curso </option>
                    {DatosRecibidos.map(curso => <option key={curso.id} value={curso.id} > {curso.Curso_Nu}  </option>)}
                </select>
                <p className="m-2" > Seleciona la materia que dictará el docente. </p>
                <select className="form-control m-2" onChange={onChange} name="materias"  >
                    <option> Materias </option>
                    {Materias.map(data => <option key={data.id}  value={data.id} > {data.N_Materia}  </option>)}
                </select>
