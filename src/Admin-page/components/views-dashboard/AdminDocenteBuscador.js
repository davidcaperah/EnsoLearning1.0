...
const BuscadorDocente = () => {
  ...
  const AgregarAulas = (Docente) => {
    ...
    cookies.set("idDoc", idEncriptado, {
      path: "/AdminDocenteCursos",
  ...
  const Buscar = async (e) => {
    ...
    if (datosRecibidos.mensaje) {
      ...
      setDatosRecibidos(datosRecibidos);
      setMensaje(datosRecibidos);
    }
    ...
    try {
      ...
      let res = await fetch(
        `${URL.servidor}/api-php-react/info_docente.php`,
        Configuracion
      );
      ...
      {Validacion ? (
        <div>
          ...
          <div className="cont-contacto-docentes">
            {DatosRecibidos.map((docente, index) => (
              <div className="cont-encotrar-info" key={docente.id}>
                <div>
                  ...
                  <div
                    className={`linea1  ${
                      index % 2 === 0 ? "linea-color1" : "linea-color2"
                    }`}
                  ></div>
                  <div
                    className={`linea2  ${
                ...
                <div className="cont-botones-info">
                  ...
                  <div onClick={() => cambiarInterfaz(docente)}>
                    {" "}
                    Ver perfil
                  </div>
                </div>
            ...
            {Mensaje.tipo === "1" ? (
              <div className="alert alert-danger m-2  w-100">
                {" "}
                {Mensaje.mensaje}{" "}
              </div>
            ) : null}
        ...
        <div>
          <div className="d-flex justify-content-start">
            <div
              className="shadow p-3 m-2 rounded-circle pointer"
