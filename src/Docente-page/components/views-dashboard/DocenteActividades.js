...
const DocenteActividades = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const respuesta = await api.post(
          "/api-php-react/info_actividad_maestro.php",
          DatosJson
        );
  ...
  useEffect(() => {
    ...
    const sendDataasig = async () => {
      ...
      const data = response.data;
      setDatosRecibidosasig(...DatosRecibidosasig, data);
    };
    sendData();
    sendDataasig();
          ...
          <div className="row contenedor-ecaluacion">
            ...
            <div className="col-md-6 col-xs-8  flex-column d-flex justify-content-end ">
              <div className="d-flex flex-row h-100 align-items-center justify-content-end mr-3 pr-5 mt-5 pt-5 ">
                <div
                  className="boton-evaluacion "
                  onClick={() => Cambiarintefaz(1, 0)}
                >
                  Ver Actividades
                </div>
                <div
                  className="boton-evaluacion offset-md-2"
                  onClick={() => Cambiarintefaz(2, 0)}
                >
                  + Crear Actividad
                </div>
              </div>
                ...
                {estadoA === 2
                  ? DatosRecibidosasig.map((actividad) =>
                      parseInt(actividad.estado_d) === 2 ? (
                        ...
                        >
                          <div className="d-flex flex-row">
                            <div className="estado-evaluaciones-vencida">
                              Estado:vencidas{" "}
                            </div>
                            <div className=""></div>
                          </div>
