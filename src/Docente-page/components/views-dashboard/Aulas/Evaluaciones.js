  ...
  return (
    <div>
      <div>
        <div className="row">
          ...
          <div className="col-8 d-flex justify-content-center align-items-end mx-5 mb-5 pb-4 con-descri-crearEvaluacion">
            <h6>
              En este espacio podras crear las evaluaciones para tus estudiantes
              con 4 diferentes tipos de pregunta
            </h6>
          </div>
        </div>
        {descriEvaluacion === 0 ? (
          <div className="row d-flex justify-content-center">
            <div className="col-7 cont-form-crearevaluacion ">
              <div
                ...
                className="volver-evaluacion"
              >{`<`}</div>
              <form onSubmit={NoRecargar} id="form">
                <p className=""> ¿Como se llamará tú evaluación? </p>
                <input
                  className="input-crearEvaluacion input-tam-CrearEvaluacion"
                  type="text"
                  name="Nombre"
                  onChange={onChange}
                />
                <p className="">
                  {" "}
                  agregar una descripcion para orientar a tus estudiantes en la
                ...
                <div className=" input-tam-medio-CrearEvaluacion">
                  <div className="mr-4">
                    <p className="mt-2">
                      {" "}
                      ¿Cuantas preguntas tendra tu evaluación?{" "}
                    </p>
                    <input
                      className="input-crearEvaluacion input-tam-CrearEvaluacion"
                      type="number"
                  ...
                  <div className="ml-4">
                    <p className="mt-2">
                      {" "}
                      asigna una fecha de entrega para la evaluacion{" "}
                    </p>
                    <input
                      className="input-crearEvaluacion input-tam-CrearEvaluacion"
                      type="date"
                  ...
                  <div className="mr-4">
                    <p className="mt-2">
                      {" "}
                      Tiempo de desarrollo para la evaluacion{" "}
                    </p>
                    <input
                      className="input-crearEvaluacion input-tam-CrearEvaluacion"
                      type="number"
                ...
                <div className="col text-center p-4">
                  <button className="boton-form-Crear-Evaluacion" type="submit">
                    {" "}
                    Agregar preguntas{" "}
                  </button>
                </div>
      ...
      {descriEvaluacion === 1 ? (
        <div>
          <div
            className="row d-flex justify-content-center aling-items-center"
            key={Campos}
          >
            <div className="col-7 cont-creador-pregunta-form">
              <h5>
                Pregunta {post_quest} de {NumeroDepreguntas}
              </h5>
              <form
                className="form-crear-preguntas"
                onSubmit={(e) => EnviarPregunta(e, Campos)}
                id={Campos + 1}
              >
                <label>Escribe aqui el enunciado de tu pregunta</label>
                {tipoPregu === 1 || tipoPregu === 3 ? (
                  <input
                    className="form-input-evaluaciones"
                    name="pregunta"
                    onChange={DatosPreguntas}
                    type="text"
                    value={CamposRespuesta.pregunta}
                  />
                ) : null}
                {tipoPregu === 2 ? (
                  <textarea
                ...
                <div className="form-selecion">
                  <div>
                    ...
                    <select
                      className="from-select-tip-pregunta"
                      name="Tipo"
                      onChange={tipoPregunta}
                    >
                      <option
                        value="1"
                        defaultChecked={tipoPregu === 1 ? true : false}
                      >
                        {" "}
                        Seleccion multiple{" "}
                      </option>
                      <option
                        value="2"
                        defaultChecked={tipoPregu === 2 ? true : false}
                      >
                        {" "}
                        Pregunta abierta{" "}
                      </option>
                  ...
                  {tipoPregu === 1 || tipoPregu === 3 ? (
                    <div>
                      <label className="label-select-cat-opciones">
                        Cantidad opciones de respuesta
                      </label>
                      {tipoPregu === 3 ? (
                        <select
                          className="from-select-cat-opciones"
                          name="cantidad"
                          onChange={cantRespuestas}
                        >
                ...
                >
                  {tipoPregu === 2 ? (
                    <h3>
                      El estudiante tendra un campo de texto para contestar
                    </h3>
                  ) : null}

                  {tipoPregu === 1 || tipoPregu === 3 ? (
                    <label>
                      Acontinuacion escribe tus opciones de respuesta
                    </label>
                  ) : (
                    ""
                  )}
                          ...
                          {opcionesRespuesta
                            ...
                            .map((el, index) => (
                              <input
                                ...
                                key={index}
                                name={"A" + index}
                                className="inputs-acti-union mb-3"
                                type="text"
                              />
                            ))}
                        </div>
                  ...
                  {tipoPregu === 1 ? (
                    <div>
                      ...
                      <select
                        className="from-select-opciones-correcta"
                        name="Correcta"
                        onChange={DatosPreguntas}
                      >
