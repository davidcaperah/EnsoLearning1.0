...
import CargarPensamientos from './cargarPensamientos';

const ElegirPeriodos = ({materia, areas}) => {
    ...
    const DiferenciarBotones = async (numero) => {
        if(numero === 0){
            setValidacion(true)
            setDatos(numero)
            document.getElementById("materias-arrow").classList.remove("Eliminar")
        }else{
            setDatos(numero)
            setValidacion(false)
            document.getElementById("materias-arrow").classList.add("Eliminar")
        }
    }
            ...
            :
                <div>
                    <div className="d-flex justify-content-start"  >
                        <div id="periodos-arrow" className="shadow p-3 pointer rounded-circle m-2 p-3" onClick={()=> DiferenciarBotones(0) } >
                    ...
                    <CargarPensamientos  materia={materia} periodo={Datos} areas={areas} />
                </div>
            }
        </div>     
    );
