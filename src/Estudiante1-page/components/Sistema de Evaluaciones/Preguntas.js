...
const Preguntas = ({ opciones, numero, id ,carga}) => {
    ...
    return (
        <div className="row p-4 d-flex col-md-12 align-items-center justify-content-center "  >
            {/* tipo 1  */}
            {opciones.Tipo === 1 ?
                <div className="btn-group btn-group-toggle col-md-12 d-flex" data-toggle="buttons">
                { Object.keys(opcion).map(e =>  
                    <label className={`btn rounded  text-white ${e+numero === correcta? 'bg-success' : 'bg-info'}  col-md-6  m-2 d-flex align-items-center align-self-center align-content-center w-100`}>
                        <input type="radio" name={numero} id={e+numero} value={e} onChange={handleChange} autoComplete="off" /> {opcion[e]}
                    </label>
                )}
                {/* {tipo 2} */}
                ...
                {/* {tipo 3} */}
                    ...
                    <div className="row" >
                        <div className="btn-group-vertical btn-group-toggle col-md-6 d-flex " data-toggle="buttons">
                            {(OnA).map(d=>
                            <label className={`btn rounded  text-white ${d.property === correcta? 'bg-success' : 'bg-info'} col-md-6  m-2 d-flex align-items-center align-self-center align-content-center w-100`}>
                                <input type="radio" name="hi" id={d.property} value={d.temp} autoComplete="off" onChange={selector} /> {d.temp}
                            </label> 
