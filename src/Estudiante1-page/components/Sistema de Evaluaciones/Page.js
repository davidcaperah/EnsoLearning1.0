...
const Page = () => {
    ...
    const datosEvaluacionEstudiantes = useSelector(state => state.datosEvaluacionEstudiantes)
    ...
    //     const datos = {
    //         d: 4,
    //         idp: datosEvaluacionEstudiantes.evaluacion.id
    //     }
    //     const datosEstu = JSON.stringify(datos)
    //     const api = axios.create({ baseURL: URL.servidor });
    //     const response = await api.post('/api-php-react/Cargar_evaluacion_m.php', datosEstu);
    ...
    return (
        <div className="p-md-5 col-sm-12" id="validar">
            <div className="mx-5 pb-4 pt-4 bg-dark col-md-9 col-sm-12 m-auto" >
                {iniciarr ?
                    <div>
                        <Cronometro minutosx={datosEvaluacionEstudiantes.evaluacion.tiempo} />
                        <div className="row shadow bg-light" >
                            <div className="col-md-12" >
                                ...
                                {preguntaUnitaria !== undefined &&
                                    <div>
                                        <Pregunta pregunta={preguntaUnitaria.pregunta} numero={valorPregunta} id={datosEvaluacionEstudiantes.evaluacion.id} />
                                    </div>
                                }
                                <Preguntas opciones={preguntaUnitaria} numero={valorPregunta} id={datosEvaluacionEstudiantes.evaluacion.id} carga={1} />
                            </div>
                        </div>

                        {
                            ...
                            <div className="d-flex justify-content-center align-items-center" >
                                <div className="d-flex jjustify-content-evenly" onClick={() => siguiente(1)} >
...

                        {
                            valorPregunta === preguntass.length &&
                            <div className="col text-center p-4">
                                <button className="btn btn-info mt-3" onClick={() => enviarExamen()}  >
                                    Enviar evaluación
                                </button>
                            </div>
                        }
                    ...
                    <div className="col-md-12" >
                        <div className="col text-center p-4">
                            <h3 className='text-light'>{datosEvaluacionEstudiantes.evaluacion.Titulo}</h3>
                            <h6 className='text-light'>{datosEvaluacionEstudiantes.evaluacion.texto}</h6>
                            <button className="btn btn-light" type="submit" onClick={() => iniciar()}> INICIAR </button>
                        </div>
                    </div>
